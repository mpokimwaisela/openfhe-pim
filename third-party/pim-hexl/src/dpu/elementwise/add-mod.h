...
static void add_mod_compute(uint64_t *out, const uint64_t *a, const uint64_t *b,
                            uint32_t n, void *ctx_) {
  uint64_t m = ((ctx_binop_t *)ctx_)->mod;
  // Debug prints removed to reduce output volume
  for (uint32_t i = 0; i < n; ++i) {
    out[i] = add_mod_u64(a[i], b[i], m);
  }
}

static void add_mod_scalar_compute(uint64_t *out, const uint64_t *a,
                                   const uint64_t *_, uint32_t n, void *ctx_) {
  ctx_scalar_t *ctx = (ctx_scalar_t *)ctx_;
  uint64_t m = ctx->mod, s = ctx->scalar;
  for (uint32_t i = 0; i < n; ++i)
